...
if Var.TG_BOT_USER_NAME_BF_HER is not None and tgbot is not None:
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            reply_pop_up_alert = "we fratm perchè cerchi di usare il mio ubot?"
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        try:
            ...
            pass
        if help_string is "":
            reply_pop_up_alert = "il creatore di {} è scemo e non ha messo la lista comandi".format(plugin_name)
        else:
